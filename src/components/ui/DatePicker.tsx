    ...
    >
              ...
              return (
                <button
                  ...
                  className={[
                    ...
                    isSelected ? 'bg-develoi-navy text-white shadow-sm hover:bg-black' : '',
                    isToday && !isSelected ? 'bg-develoi-navy/5 border border-develoi-navy/20 text-develoi-navy' : '',
                    !isSelected && !isToday ? 'hover:bg-develoi-navy/5 hover:text-develoi-navy' : '',
        ...
        <div className="grid grid-cols-3 gap-2 p-3 h-[260px]">
          {monthNames.map((m, i) => (
            <button key={i} type="button" onClick={() => { setViewDate(new Date(viewDate.getFullYear(), i, 1)); setMode('day'); }}
              className={`rounded-xl flex items-center justify-center font-bold text-xs transition-colors border ${viewDate.getMonth() === i ? 'bg-develoi-navy text-white border-transparent' : 'bg-zinc-50 text-zinc-700 hover:bg-develoi-navy/5 border-zinc-200'}`}>
          ...
          {Array.from({ length: 12 }).map((_, i) => {
            ...
            return (
              <button key={y} type="button" onClick={() => { setViewDate(new Date(y, viewDate.getMonth(), 1)); setMode('month'); }}
                className={`rounded-xl flex items-center justify-center font-bold text-xs transition-colors border ${viewDate.getFullYear() === y ? 'bg-develoi-navy text-white border-transparent' : 'bg-zinc-50 text-zinc-700 hover:bg-develoi-navy/5 border-zinc-200'}`}>
                {y}
              </button>
            );
          })}
      ...
      <div className="flex items-center justify-between border-t border-zinc-100 px-3 py-2 bg-zinc-50/80">
        <button type="button" onClick={() => { const now = new Date(); setViewDate(now); if (mode === 'day') handleSelectDate(now); else setMode('day'); }}
          className="rounded-lg px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-zinc-500 hover:text-develoi-navy hover:bg-develoi-navy/5 transition-colors">
          Hoje
        </button>
        ...
        {renderTrigger ? (
          ...
          <>
            ...
            {showIcon && variant !== 'ghost' && (
              <CalendarDays size={15} className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-400 pointer-events-none group-focus-within:text-develoi-navy transition-colors" />
            )}
          </>
        )}
      </div>
